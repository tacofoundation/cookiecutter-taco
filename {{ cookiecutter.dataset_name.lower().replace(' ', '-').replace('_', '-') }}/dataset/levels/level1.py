...
- level1 samples are FILEs with actual data (GeoTIFF, PNG, bytes, etc.)
{% else %}This is the SECOND level of your TACO hierarchy. Each sample here is contained
within a level0 sample.

Structure:
- level0 (root) → level1 (this level) → level2 → ... → level{{ cookiecutter.max_levels }} (leaves)
- level1 samples are FOLDERs that contain level2 tortillas
{% endif %}

How to use:
{% if cookiecutter.max_levels|int == 1 %}1. Copy build_sample_example() and rename it (e.g., build_sample_image_001)
...
{% else %}1. Copy build_sample_example() and rename it (e.g., build_sample_date_001)
2. Implement your logic to build the child tortilla from level2
...
{% if cookiecutter.max_levels|int > 1 %}from dataset.levels import level2{% endif %}
...
# =============================================================================
{% if cookiecutter.max_levels|int == 1 %}# Each function builds ONE FILE sample at this level.
...
{% if cookiecutter.max_levels|int == 1 %}    # Example: Create in-memory data
    data = b"test_data"
    
    sample = Sample(id="example", path=data)
{% else %}    child_tortilla = level2.build()
...
{% endif %}    
    ...
    #   ))
{% if cookiecutter.max_levels|int == 1 %}    #
...
if __name__ == "__main__":
    ...
    print("Running level1 validation...")    
