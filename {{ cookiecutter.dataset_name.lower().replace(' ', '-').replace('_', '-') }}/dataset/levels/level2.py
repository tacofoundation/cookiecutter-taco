...
{% if cookiecutter.max_levels|int == 2 %}# from tacotoolbox.sample.extensions.tacotiff import Header
...
{% if cookiecutter.max_levels|int == 2 %}# Sample builders - one function per file type
...
def build_sample_scene(ctx: dict) -> Sample:
    ...
    sample = Sample(id="scene", path=child_tortilla)
...
{% endif %}
    ...
    print(f"Building level2 with {len(contexts)} contexts...")
    for ctx in contexts:
        tortilla = build(ctx)
